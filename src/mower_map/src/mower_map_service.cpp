...
void DelteLocalFile(const std::string &file) {
  if (remove(file.c_str()) == 0) {
    ROS_INFO("detele succusss");
  } else {
    ROS_INFO("detele  fail");
  }
};

bool ClearMowingAreaMapData(std_srvs::Empty::Request &request,
                            std_srvs::Empty::Response &response) {
  mowing_areas.clear();
  navigation_areas.clear();
  ROS_INFO("delte bag file %s", map_path);
  //   DelteLocalFile(map_path);
  buildMap();
  return true;
}
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer delete_area_srv_bag = n.advertiseService(
      "mower_map_service/delete_mowing_area_bag", ClearMowingAreaMapData);
  // 读取bag文件添加地图
