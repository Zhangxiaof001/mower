...
// This can be set to true by anyone but will only be set to false by the idle
// state on exit
...
void checkSafety(const ros::TimerEvent &timer_event) {
  ...
  if (ros::Time::now() - odom_time > ros::Duration(1.0)) {
    ...
    ROS_ERROR_STREAM("EMERGENCY: odometry values stopped. dt was: "
                     << (ros::Time::now() - odom_time));
    return;
  }

  // check if status is current. if not, we have a problem since it contains
  // wheel ticks and so on -> emergency
  if (ros::Time::now() - status_time > ros::Duration(0.5)) {
    setEmergencyMode(true);
    ROS_ERROR_STREAM("EMERGENCY: status values stopped. dt was: "
  ...
  bool gpsGood =
      last_odom.pose.covariance[0] < 0.05 && last_odom.pose.covariance[0] > 0;
  if (gpsGood || last_config.ignore_gps_errors) {
    last_good_gps = ros::Time::now();
  }

  if (currentBehavior != nullptr && currentBehavior->needs_gps() &&
      ros::Time::now() - last_good_gps >
          ros::Duration(last_config.gps_timeout)) {
  ...
  setMowerEnabled(currentBehavior != nullptr &&
...
int main(int argc, char **argv) {
  ...
  current_state_pub =
      n->advertise<std_msgs::String>("mower_logic/current_state", 100, true);
  ...
  // 割草机状态 - mower_comms发布
  // ros::Subscriber status_sub = n->subscribe("/mower/status", 0,
  // statusReceived,
  //                                          ros::TransportHints().tcpNoDelay(true));
  // ros::Subscriber odom_sub = n->subscribe("/mower/odom", 0, odomReceived,
  // ros::TransportHints().tcpNoDelay(true));
  ...
  ros::Subscriber reach_goal_sub =
      n->subscribe("/local_planner/reach_goal", 0, ReachGoalReceived,
                   ros::TransportHints().tcpNoDelay(true));
