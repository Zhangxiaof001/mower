...
PLUGINLIB_EXPORT_CLASS(ftc_local_planner::FTCPlanner,
...
void FTCPlanner::initialize(std::string name, tf2_ros::Buffer *tf,
                            costmap_2d::Costmap2DROS *costmap_ros) {
  ros::NodeHandle private_nh("~/" + name);

  progress_server = private_nh.advertiseService("planner_get_progress",
                                                &FTCPlanner::getProgress, this);

  global_point_pub =
      private_nh.advertise<geometry_msgs::PoseStamped>("global_point", 1);
  global_plan_pub =
      private_nh.advertise<nav_msgs::Path>("global_plan", 1, true);
  ...
  dynamic_reconfigure::Server<FTCPlannerConfig>::CallbackType cb =
      boost::bind(&FTCPlanner::reconfigureCB, this, _1, _2);
...
bool FTCPlanner::setPlan(const std::vector<geometry_msgs::PoseStamped> &plan) {
  ...
  if (global_plan.size() > 2) {
    ...
    global_plan[global_plan.size() - 2].pose.orientation =
        global_plan[global_plan.size() - 3].pose.orientation;
...
double FTCPlanner::distanceLookahead() {
  ...
  for (uint32_t i = current_index + 1; i < global_plan.size(); i++) {
    ...
    if (abs(rot2.angularDistance(current_rot)) >
        config.speed_fast_threshold_angle * (M_PI / 180.0)) {
      break;
    }
  }

  return (last_straight_point.translation() -
          current_control_point.translation())
      .norm();
}

uint32_t FTCPlanner::computeVelocityCommands(
  ...
  // First, we update the control point if needed. This is needed since we need
  // the local_control_point to calculate the next state.
...
FTCPlanner::PlannerState FTCPlanner::update_planner_state() {
  switch (current_state) {
    ...
    case WAITING_FOR_GOAL_APPROACH: {
      ...
      if (time_in_current_state() > config.goal_timeout) {
        ROS_WARN_STREAM(
            "Could not reach goal position. Attempting final rotation "
            "anyways.");
        return POST_ROTATE;
      }
...
void FTCPlanner::update_control_point(double dt) {
  switch (current_state) {
    ...
    case FOLLOWING: {
      ...
      if (speed > current_movement_speed) {
        ...
        if (current_movement_speed > speed) current_movement_speed = speed;
      ...
      while (angle_to_move > 0 && distance_to_move > 0 &&
             current_index < global_plan.size() - 2) {
        ...
        double remaining_distance_to_next_pose =
            pose_distance * (1.0 - current_progress);
        double remaining_angular_distance_to_next_pose =
            pose_distance_angular * (1.0 - current_progress);

        if (remaining_distance_to_next_pose < distance_to_move &&
            remaining_angular_distance_to_next_pose < angle_to_move) {
          // we need to move further than the remaining distance_to_move. Skip
          // to the next point and decrease distance_to_move.
          current_progress = 0.0;
          current_index++;
          ...
          double current_progress_distance =
              (pose_distance * current_progress + distance_to_move) /
              pose_distance;
          double current_progress_angle =
              (pose_distance_angular * current_progress + angle_to_move) /
              pose_distance_angular;
          current_progress =
              fmin(current_progress_angle, current_progress_distance);
      ...
      result.translation() =
          (1.0 - current_progress) * trans1 + current_progress * trans2;
      result.linear() = rot1.slerp(current_progress, rot2).toRotationMatrix();

      current_control_point = result;
    } break;
    case POST_ROTATE:
      tf2::fromMsg(global_plan[global_plan.size() - 1].pose,
  ...
  auto map_to_base = tf_buffer->lookupTransform("base_link", "map", ros::Time(),
                                                ros::Duration(1.0));
  ...
  angle_error = GetYaw(Eigen::Quaterniond(local_control_point.rotation()));
  auto angle_error1 =
      local_control_point.rotation().eulerAngles(0, 1, 2).z();  //+3.1415926/2;
  // if(angle_error>=3.1415)angle_error= angle_error-2*3.1415926;

  ROS_INFO("%f,%f,%f,%f", lon_error, lat_error, angle_error, angle_error1);
}

void FTCPlanner::calculate_velocity_commands(
    double dt, geometry_msgs::TwistStamped &cmd_vel) {
  ...
  if (current_state == FOLLOWING) {
    double lin_speed = lon_error * config.kp_lon + i_lon_error * config.ki_lon +
                       d_lon * config.kd_lon;
  ...
  if (current_state == FOLLOWING) {
    double ang_speed = angle_error * config.kp_ang +
                       i_angle_error * config.ki_ang + d_angle * config.kd_ang +
                       lat_error * config.kp_lat + i_lat_error * config.ki_lat +
                       d_lat * config.kd_lat;
...
bool FTCPlanner::getProgress(PlannerGetProgressRequest &req,
                             PlannerGetProgressResponse &res) {
